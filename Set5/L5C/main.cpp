/* CSCI 261: Assignment 5: Lab 5C - Recursion I: Towers of Hanoi
 *
 * Author: Brandon Ching
 * Resources: N/A
 *
 * Description: using recursion to solve the tower of hanoi
 */
 ...
 *
 * @note Outputs the series of moves to standard otu to solve the problem for N
 * disks
 */
void towers_of_hanoi(const int N, const int START_PEG, const int TARGET_PEG,
                     const int SPARE_PEG) {
  // base case - if there are no more disks to move, then we are done!  do
  // nothing
  if (N == 0) {
    return;
  }
  ...
  cout << "Move disk " << N << " from " << START_PEG << " to " << TARGET_PEG
       << endl;
...
int main() {
  ...
  const int SPARE_PEG =
      1;  // 1 - ID of the peg that can be used for temp storage
